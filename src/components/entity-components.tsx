...
type EntityHeaderProps = {
  title: string;
  description?: string;
  newBottonLabel: string;
  disabled?: boolean;
  isCreating?: boolean;
} & (
  | { onNew: () => void; newBottonHref?: never }
  | { newBottonHref: string; onNew?: never }
  | { onNew?: never; newBottonHref?: never }
);
    ...
    <div className="flex flex-row items-center justify-between gap-x-4">
      ...
      {newBottonHref && !onNew && (
        <Button size="sm" asChild>
          <Link href={newBottonHref} prefetch>
            <PlusIcon className="size-4" />
            {newBottonLabel}
          </Link>
        </Button>
...
export const EntitySearch = ({ value, onChange, placeholder = 'Search' }: EntitySearchProps) => {
  return (
    <div className="relative  ml-auto">
      <SearchIcon className="size-3.5 absolute left-3 top-1/2 -translate-y-0.5 text-muted-foreground" />
      <Input
